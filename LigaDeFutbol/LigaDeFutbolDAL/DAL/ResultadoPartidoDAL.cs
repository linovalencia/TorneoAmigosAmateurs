...
namespace LigaDeFutbolDAL
{
    public class ResultadoPartidoDAL
    {
        ...
        public static List<PartidoDTO> obtenerPartidosDeFechaXPorId(int idFechaCampeonato)
        {
            ...
            string consultaSql = @"SELECT p.*, cl.nombreClub as nombreClubLocal, cv.nombreClub as nombreClubVisitante 
                                FROM partido p INNER JOIN club cl on cl.idClub=p.idClubLocal INNER JOIN club cv on cv.idClub=p.idClubVisitante
                                WHERE idFechaCampeonato=@idFechaCampeonato";
            SqlConnection cnn = new SqlConnection(DALBase.StringConexion);
            ...
            try
            {
                ...
                SqlCommand cmd = new SqlCommand(consultaSql, cnn);
                cmd.Parameters.AddWithValue(@"idFechaCampeonato", idFechaCampeonato);
                SqlDataReader dr = cmd.ExecuteReader();

                while (dr.Read())
                {
                   ResultadoPartidoDTO p = new ResultadoPartidoDTO();
                    ...
                    p.nombreClubLocal = (dr["nombreClubLocal"].ToString());
                    p.nombreClubVisitante = (dr["nombreClubVisitante"].ToString());
